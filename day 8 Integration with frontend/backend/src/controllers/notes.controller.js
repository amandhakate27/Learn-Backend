...
const singleEntityUpdateController = async (req, res)=>{
    try{
        let noteId = req.params.id;
        let body = req.body;
        let updatedNote = await notesModel.findByIdAndUpdate(noteId, body, {new : true}); // quickly update specific fields of a note by id using new : true to return the updated document
        return res.status(200).json({
            message : "Note updated successfully",
            data : updatedNote
        })
...
module.exports = {createNotessController, getAllNotesController, getSingleNoteController , updateNoteController, deleteNoteController , singleEntityUpdateController};